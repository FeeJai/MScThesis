...
Like all hyperbolic operations, the Tangens hyperbolicus is a generalization of the trigonometric function onto the complex number space so that $\text{tan}(ix) = i * \text{tanh}(x)$. Similar to the sigmoid function, the TanH-function is constrained by two horizontal asymptotes, but the range of output is $(-1; 1)$. The function can be calculated as follows:
...
Since the lower bound is not very
